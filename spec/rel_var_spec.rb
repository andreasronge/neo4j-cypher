...
describe Neo4j::Cypher::RelVar do
  ...
  context "rel(:foo, :bar, :baaz)" do
    before { clause_list.stub(:create_variable).and_return(:v1) }
    subject { Neo4j::Cypher::RelVar.new(clause_list, [:foo, :bar, :baaz]) }
    its (:match_value) {should == ':`foo`|`bar`|`baaz`'}
    its (:var_name) {should == :v1}
    its (:rel_type) { should == '`foo`|`bar`|`baaz`'}
  end
